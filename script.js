...
const LIBRARY_ENDPOINTS={
  books:(q)=>`https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(q)}&maxResults=12&printType=books`,
  movies:(q)=>`https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(q)}&language=ko&uselang=ko&format=json&origin=*&limit=8`
};
...
function resetLibraryForm(type){
 ...
 $("#libraryModalLabel").textContent=type==="books"?"BOOK":"MOVIE";$("#libraryModalTitle").textContent=type==="books"?"책 추가":"영화 추가";
...
async function searchBooks(q){
 const r=await fetch(LIBRARY_ENDPOINTS.books(q));if(!r.ok)throw Error("book search failed");const j=await r.json();
 const raw=(j.items||[]).map(x=>{const i=x.volumeInfo||{},im=i.imageLinks||{};return {id:x.id,type:"books",title:cleanText(i.title),subtitle:cleanText(i.subtitle),creator:cleanText((i.authors||[]).join(", ")),image:coverUrl(im.extraLarge||im.large||im.medium||im.thumbnail||im.smallThumbnail),altTitle:"",source:"Google Books"}}).filter(x=>x.title);
 for(const item of raw){item.coverChoices=[...new Set(raw.filter(y=>y.title.toLowerCase()===item.title.toLowerCase()&&y.image).map(y=>y.image))].slice(0,5)}
 return raw;
}
async function wikidataEntity(id){const r=await fetch(`https://www.wikidata.org/w/api.php?action=wbgetentities&ids=${encodeURIComponent(id)}&props=labels|claims&languages=ko|en&format=json&origin=*`);if(!r.ok)throw Error("entity failed");const j=await r.json();return j.entities?.[id]||null}
async function searchMovies(q){
 const r=await fetch(LIBRARY_ENDPOINTS.movies(q));if(!r.ok)throw Error("movie search failed");const j=await r.json();
 const arr=[];
 for(const x of (j.search||[]).slice(0,8)){
  try{
   const e=await wikidataEntity(x.id), labels=e?.labels||{}, claims=e?.claims||{};
   const titleKo=cleanText(labels.ko?.value),titleEn=cleanText(labels.en?.value);const title=titleKo||titleEn||cleanText(x.label);const alt=titleKo&&titleEn&&titleKo!==titleEn?titleEn:"";
   let creator="";const p57=claims.P57?.[0]?.mainsnak?.datavalue?.value?.id;if(p57){const de=await wikidataEntity(p57);creator=cleanText(de?.labels?.ko?.value||de?.labels?.en?.value)}
   const coverChoices=(claims.P18||[]).map(c=>c.mainsnak?.datavalue?.value).filter(Boolean).slice(0,5).map(v=>`https://commons.wikimedia.org/wiki/Special:Redirect/file/${encodeURIComponent(v)}?width=500`);
   const image=coverChoices[0]||"";
   arr.push({id:x.id,type:"movies",title,altTitle:alt,creator,image,coverChoices,source:"Wikidata"});
  }catch{}
 }
 return arr;
}
function renderLibraryResults(results,type){
 ...
 box.querySelectorAll(".library-result").forEach((el,i)=>el.onclick=()=>selectLibraryResult(results[i],type));
...
async function runLibrarySearch(){
 ...
 try{const results=type==="books"?await searchBooks(q):await searchMovies(q);renderLibraryResults(results,type);$("#libraryFetchMessage").textContent=results.length?`${results.length}개의 결과를 찾았어요. 정확한 작품을 선택해 주세요.`:"검색 결과가 없습니다."}catch(e){console.error(e);$("#libraryFetchMessage").textContent="자동 검색에 실패했어요. 아래 정보를 직접 입력할 수 있어요."}finally{btn.disabled=false;btn.textContent="자동 검색"}
}
