...
function colorForEvent(e){
 // 겹치는 일정은 lane 순서에 따라 서로 다른 색 계열을 사용해요.
 // 첫 번째는 산뜻한 그린, 두 번째는 핑크, 세 번째는 블루 계열.
 const palette=["#A8D5BA","#F2B5C8","#AFC8EA","#F3D38A","#C8B5E8","#F2B08F"];
 return palette[Math.min(Math.max(e._lane||0,0),palette.length-1)];
}
function renderCal(){
 ...
 box.innerHTML=es.length?es.map((e,i)=>{
   const color=colorForEvent({...e,_lane:es.findIndex(x=>x===e)});
