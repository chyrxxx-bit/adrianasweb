...
function eventColorMap(){
 ...
 const map=new Map();
 events.sort((a,b)=>eventStart(a).localeCompare(eventStart(b)) || eventEnd(a).localeCompare(eventEnd(b)) || String(a.title).localeCompare(String(b.title)) || eventIdentity(a).localeCompare(eventIdentity(b)));
 // Every distinct schedule gets its own color. A multi-day series keeps the same color on every date.
 events.forEach((e,index)=>map.set(eventIdentity(e),EVENT_PALETTE[index%EVENT_PALETTE.length]));
 return map;
}
