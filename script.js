...
async function fetchLibraryMeta(url,type){const u=safeUrl(url);if(!u)throw Error("invalid url");return await fetchJson(`${LIBRARY_ENDPOINT}?type=metadata&kind=${encodeURIComponent(type)}&url=${encodeURIComponent(u)}`)}
...
openLibraryDialog=function(type){resetLibraryForm(type);const d=$("#libraryDialog");try{d.showModal()}catch{d.setAttribute("open","")}setTimeout(()=>$("#libraryUrl")?.focus(),60)};
...
$("#loginForm").onsubmit=async e=>{e.preventDefault();const email=$("#authEmail").value.trim(),pw=$("#authPassword").value;$("#authMessage").textContent="로그인 중…";try{await setPersistence(auth,browserLocalPersistence);await signInWithEmailAndPassword(auth,email,pw);$("#authMessage").textContent=""}catch(err){console.error(err);const m={"auth/invalid-credential":"이메일 또는 비밀번호가 맞지 않아요.","auth/user-not-found":"가입된 계정을 찾지 못했어요.","auth/wrong-password":"비밀번호가 맞지 않아요.","auth/invalid-email":"이메일 형식을 확인해 주세요.","auth/too-many-requests":"잠시 후 다시 시도해 주세요."};$("#authMessage").textContent=m[err?.code]||"로그인에 실패했어요. 잠시 후 다시 시도해 주세요."}};
$("#signupBtn").onclick=async()=>{try{await setPersistence(auth,browserLocalPersistence);await createUserWithEmailAndPassword(auth,$("#authEmail").value.trim(),$("#authPassword").value);$("#authMessage").textContent=""}catch(err){console.error(err);$("#authMessage").textContent="회원가입에 실패했어요. 이메일 형식과 비밀번호(6자 이상)를 확인해 주세요."}};
$("#logoutBtn").onclick=()=>signOut(auth);
onAuthStateChanged(auth,async user=>{showAuth(user);if(!user){S.ready=false;return}loadLocal();renderCal();renderWishes();renderLibrary("books");renderLibrary("movies");try{await cloudLoad();S.ready=true;renderCal();renderWishes();renderLibrary("books");renderLibrary("movies")}catch(e){console.error(e);updateSync("로그인됨 · 온라인 저장 실패");S.ready=true}});
