...
function openDeleteDialog(type,id,message){
 pendingDelete={type,id};
 $("#deleteMessage").textContent=message;
 const d=$("#deleteDialog");
 try{d.showModal()}catch{d.setAttribute("open","")}
}
function closeDeleteDialog(){const d=$("#deleteDialog");if(d.open)d.close();else d.removeAttribute("open");pendingDelete=null}
$("#deleteCancel").onclick=closeDeleteDialog;
$("#deleteConfirm").onclick=async()=>{
 if(!pendingDelete)return;
 const {type,id}=pendingDelete;
 if(type==="wish")S.wishes=S.wishes.filter(x=>x.id!=id);
 else S[type]=S[type].filter(x=>x.id!=id);
 localBackup();
 closeDeleteDialog();
 if(type==="wish")renderWishes();else renderLibrary(type);
...
function renderWishes(){
 ...
 document.querySelectorAll(".delete-btn").forEach(b=>b.onclick=()=>openDeleteDialog("wish",b.dataset.delete,"이 위시를 삭제할까요?"));
}
...
function resetLibraryForm(type){
 ...
 document.querySelectorAll(".title-priority-btn").forEach(b=>b.classList.toggle("active",b.dataset.titlePriority==="ko"));
}
...
function saveLibraryRecord(type){
 const ko=$("#libraryKoreanTitle").value.trim();
 const foreign=$("#libraryForeignTitle").value.trim();
 const creator=$("#libraryManualCreator").value.trim();
 const image=$("#libraryImageFile").dataset.data||"";
 if(!ko&&!foreign){alert("한국어 제목 또는 외국어 제목을 하나 이상 입력해 주세요.");return null}
 const priority=$("#libraryForm").dataset.titlePriority || (ko?"ko":"foreign");
 return{id:Date.now(),url:"",title:priority==="foreign"?(foreign||ko):(ko||foreign),altTitle:ko&&foreign?(priority==="foreign"?ko:foreign):"",titleKo:ko,titleForeign:foreign,titlePrimary:priority,creator,image,status:$("#libraryStatus").value,createdAt:Date.now()}
}
...
$("#libraryForm").onsubmit=async e=>{
 ...
 if(!rec)return;
 S[type].unshift(rec);
 try{localBackup()}catch(err){console.error("Local save error:",err)}
 renderLibrary(type);
 const dlg=$("#libraryDialog");
 if(dlg.open) dlg.close(); else dlg.removeAttribute("open");
 try{await cloudSave()}catch(err){console.error("Cloud save error:",err);updateSync("기기에 저장됨")}
};
...
function renderLibrary(type){const status=S.libraryStatus[type],q=$(type==="books"?"#bookSearch":"#movieSearch").value.trim().toLowerCase(),arr=S[type].filter(x=>x.status===status&&(!q||librarySearchMatch(x,q))),grid=$(type==="books"?"#bookGrid":"#movieGrid");grid.innerHTML=arr.length?arr.map(x=>{
 const priority=x.titlePrimary || (x.titleKo&&x.titleForeign ? "ko" : x.titleForeign ? "foreign" : "ko");
 const main=priority==="foreign"?(x.titleForeign||x.titleKo||x.title||""):(x.titleKo||x.titleForeign||x.title||"");
 const other=x.titleKo&&x.titleForeign?(priority==="foreign"?x.titleKo:x.titleForeign):"";
 const nextStatus=type==="books"?(status==="to-read"?"read":"to-read"):(status==="to-watch"?"watched":"to-watch");
 const nextLabel=type==="books"?(status==="to-read"?"READ":"TO READ"):(status==="to-watch"?"WATCH":"TO WATCH");
 return`<article class="wish library-card" data-library-card="${x.id}" data-library-type="${type}" tabindex="0" role="button" aria-label="${escapeAttr(main)} 카드 삭제"><div class="wish-img library-image-placeholder">${x.image?`<img src="${escapeAttr(x.image)}" loading="lazy" alt="" onerror="this.style.display='none'">`:`<span>NO IMAGE</span>`}</div><div class="wish-body"><div class="library-title-row"><h3 class="library-main-title">${escapeHtml(main)}</h3>${other?`<div class="library-alt-title">${escapeHtml(other)}</div>`:""}</div><div class="library-creator">${escapeHtml(x.creator||"정보 없음")}</div><div class="library-card-footer"><button class="library-status-toggle" data-library-status-toggle="${x.id}" data-library-type="${type}" data-next-status="${nextStatus}">${nextLabel}</button></div></div></article>`
}).join(""): '<p class="empty">등록된 작품이 없습니다.</p>';
grid.querySelectorAll("[data-library-status-toggle]").forEach(b=>b.onclick=async e=>{e.stopPropagation();const item=S[b.dataset.libraryType].find(x=>x.id==b.dataset.libraryStatusToggle);if(!item)return;item.status=b.dataset.nextStatus;localBackup();renderLibrary(b.dataset.libraryType);try{await cloudSave()}catch(err){console.error(err);updateSync("기기에 저장됨")}});
grid.querySelectorAll("[data-library-card]").forEach(card=>{
 const remove=async()=>{
  const type=card.dataset.libraryType,id=card.dataset.libraryCard,item=S[type].find(x=>x.id==id);
  if(!item)return;
  const label=item.titleKo||item.titleForeign||item.title||"이 작품";
  openDeleteDialog(type,id,`‘${label}’을(를) 삭제할까요?`);
 };
 card.onclick=e=>{if(e.target.closest("button,a,input,select"))return;remove()};
 card.onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();remove()}}
})}
...
document.querySelectorAll("[data-title-priority]").forEach(b=>b.onclick=()=>{const form=$("#libraryForm");form.dataset.titlePriority=b.dataset.titlePriority;document.querySelectorAll(".title-priority-btn").forEach(x=>x.classList.toggle("active",x===b))});$("#bookSearch").oninput=()=>renderLibrary("books");$("#movieSearch").oninput=()=>renderLibrary("movies");
