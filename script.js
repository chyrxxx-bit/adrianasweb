...
// ---------- Books / Movies library ----------
// Metadata is intentionally manual. Only cover/poster images are searched automatically.
const LIBRARY_ENDPOINTS={
  books:(q)=>`https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(q)}&maxResults=20&printType=books&orderBy=relevance`,
  moviesKR:(q)=>`https://itunes.apple.com/search?term=${encodeURIComponent(q)}&entity=movie&country=KR&media=movie&limit=30`,
  moviesUS:(q)=>`https://itunes.apple.com/search?term=${encodeURIComponent(q)}&entity=movie&country=US&media=movie&limit=30`,
  tv:(q)=>`https://api.tvmaze.com/search/shows?q=${encodeURIComponent(q)}`
};
...
function titleScore(title,q){
 ...
 const grams=v=>new Set([...v].map((_,i)=>v.slice(i,i+2)).filter(x=>x.length===2));const a=grams(t),b=grams(n);let inter=0;for(const x of a)if(b.has(x))inter++;const u=new Set([...a,...b]).size,j=u?inter/u:0;return j>=.35?500+Math.round(j*200):0;
}
function dedupeCovers(arr){const m=new Map();for(const x of arr){if(!x?.image)continue;const k=x.image.split('?')[0];if(!m.has(k))m.set(k,x)}return [...m.values()].sort((a,b)=>(b.score||0)-(a.score||0)).slice(0,12)}
async function searchBookCovers(q){
 const r=await fetch(LIBRARY_ENDPOINTS.books(q),{cache:"no-store"});if(!r.ok)throw Error("책 표지 검색에 실패했어요.");const j=await r.json();
 return (j.items||[]).filter(x=>x?.volumeInfo?.printType!=="MAGAZINE").map(x=>{const i=x.volumeInfo||{},im=i.imageLinks||{};return {title:cleanText(i.title),image:coverUrl(im.extraLarge||im.large||im.medium||im.thumbnail||im.smallThumbnail),source:"Google Books",score:titleScore(i.title,q)}}).filter(x=>x.image&&x.title);
}
async function searchMovieCovers(q){
 const out=[];
 const calls=[fetch(LIBRARY_ENDPOINTS.moviesKR(q),{cache:"no-store"}).then(r=>r.ok?r.json():{results:[]}),fetch(LIBRARY_ENDPOINTS.moviesUS(q),{cache:"no-store"}).then(r=>r.ok?r.json():{results:[]}),fetch(LIBRARY_ENDPOINTS.tv(q),{cache:"no-store"}).then(r=>r.ok?r.json():[])];
 const [kr,us,tv]=await Promise.allSettled(calls);
 for(const z of [kr,us])if(z.status==='fulfilled')for(const x of (z.value.results||[])){if(x?.wrapperType!=="track"||x?.kind!=="feature-movie")continue;out.push({title:cleanText(x.trackName),image:coverUrl(x.artworkUrl100||x.artworkUrl60),source:"Apple",score:titleScore(x.trackName,q)});}
 if(tv.status==='fulfilled')for(const x of (tv.value||[])){const show=x?.show;if(!show?.name)continue;out.push({title:cleanText(show.name),image:coverUrl(show.image?.original||show.image?.medium),source:"TV",score:titleScore(show.name,q)});}
 return out;
}
async function findLibraryCovers(type, titles){
 const qs=[...new Set(titles.map(cleanText).filter(Boolean))];if(!qs.length)return [];
 let raw=[];for(const q of qs){try{raw.push(...(type==="books"?await searchBookCovers(q):await searchMovieCovers(q)))}catch(e){console.warn(e)}}
 return dedupeCovers(raw);
}
function renderLibraryResults(results){
 const box=$("#libraryResults");box.classList.add("has-results");box.innerHTML=results.length?results.map((r,i)=>`<button type="button" class="library-result cover-only-result" data-cover-index="${i}"><img src="${escapeAttr(r.image)}"><span><strong>${escapeHtml(r.title)}</strong><em>${escapeHtml(r.source||"")}</em></span></button>`).join(""):'<div class="empty">맞는 표지나 포스터를 찾지 못했어요.</div>';
...
function renderCoverChoices(item){
 ...
 box.classList.remove("hidden");box.innerHTML='<span>선택한 표지 / 포스터</span>'+imgs.map((im,i)=>`<button type="button" class="cover-choice ${i===0?"active":""}" data-image="${escapeAttr(im)}"><img src="${escapeAttr(im)}"></button>`).join("");
...
async function runLibrarySearch(){
 ...
 try{const results=await findLibraryCovers(type,titles);renderLibraryResults(results);$("#libraryFetchMessage").textContent=results.length?`${results.length}개의 표지 / 포스터를 찾았어요.`:"표지 / 포스터를 찾지 못했어요. 이미지 링크를 직접 넣을 수 있어요."}
 catch(e){console.error(e);$("#libraryFetchMessage").textContent="자동 검색에 실패했어요. 이미지 링크를 직접 넣어도 돼요."}
 finally{btn.disabled=false;btn.textContent="표지 / 포스터 찾기"}
}
function openLibraryDialog(type){resetLibraryForm(type);const d=$("#libraryDialog");try{d.showModal()}catch{d.setAttribute("open","")}setTimeout(()=>$("#libraryKoreanTitle")?.focus(),60)}
...
$("#libraryForm").onsubmit=async e=>{e.preventDefault();const type=libraryTypeFromDialog(),ko=cleanText($("#libraryKoreanTitle").value),foreign=cleanText($("#libraryForeignTitle").value),creator=cleanText($("#libraryManualCreator").value),image=cleanText($("#libraryManualImage").value)||$("#libraryForm").dataset.selectedImage||"assets/flower.jpg";if(!ko&&!foreign){alert("한국어 제목 또는 외국어 제목을 하나 이상 입력해 주세요.");return}const rec={id:Date.now(),title:ko||foreign,altTitle:ko&&foreign?foreign:"",titleKo:ko,titleForeign:foreign,creator,image,status:$("#libraryStatus").value,createdAt:Date.now()};S[type].unshift(rec);await cloudSave();$("#libraryDialog").close();renderLibrary(type)};
