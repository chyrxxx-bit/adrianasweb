...
// WISH와 같은 방식: 링크를 넣고 정보를 가져오면 제목/작가·감독/표지를 채웁니다.
function libraryTypeFromDialog(){return $("#libraryDialog").dataset.type||"books"}
async function fetchLibraryMeta(url,type){
 const u=safeUrl(url);if(!u)throw Error("invalid url");
 // GitHub Pages는 Netlify Functions를 실행할 수 없으므로 WISH와 같은 Microlink 방식으로 가져옵니다.
 const endpoint="https://api.microlink.io/?url="+encodeURIComponent(u)+
   "&data.title.selector=title"+
   "&data.image.selector=meta[property='og:image'],meta[name='twitter:image']"+
   "&data.creator.selector=meta[name='author'],meta[property='book:author'],[rel='author']";
 const r=await fetch(endpoint,{cache:"no-store"});if(!r.ok)throw Error("metadata fetch failed");
 const j=await r.json(),d=j.data||{};
 let title=getNestedValue(d.title).trim();
 let image=getNestedValue(d.image?.url||d.image).trim();
 let creator=getNestedValue(d.creator).trim();
 // 페이지 메타데이터에 제작자 정보가 없으면 제목으로 공개 검색 API를 보완합니다.
 if(type==="books" && title){
   try{
     const q=encodeURIComponent(title.replace(/\s*[|｜–—-]\s*.*$/,''));
     const br=await fetch("https://www.googleapis.com/books/v1/volumes?q=intitle:"+q+"&maxResults=5&langRestrict=ko",{cache:"no-store"});
     if(br.ok){const bj=await br.json(),v=(bj.items||[])[0]?.volumeInfo||{};creator=creator||((v.authors||[]).join(", "));image=image||v.imageLinks?.thumbnail||v.imageLinks?.smallThumbnail||"";title=title||v.title||"";}
   }catch{}
 }
 if(type==="movies" && title && !creator){
   try{
     const q=encodeURIComponent(title.replace(/\s*[|｜–—-]\s*.*$/,''));
     const mr=await fetch("https://itunes.apple.com/search?term="+q+"&country=KR&media=movie&entity=movie&limit=5",{cache:"no-store"});
     if(mr.ok){const mj=await mr.json(),v=(mj.results||[])[0];if(v){creator=v.artistName||"";image=image||v.artworkUrl100?.replace(/100x100/g,"600x600")||"";title=v.trackName||title;}}
   }catch{}
 }
 if(!title&&!image&&!creator)throw Error("no metadata");
 return{title,creator,image,source:"link"};
}
function resetLibraryForm(type){const f=$("#libraryForm");f.reset();$("#libraryDialog").dataset.type=type;$("#libraryModalLabel").textContent=type==="books"?"BOOK":"MOVIE / DRAMA";$("#libraryModalTitle").textContent=type==="books"?"책 추가":"영화 / 드라마 추가";$("#libraryStatus").innerHTML=type==="books"?'<option value="to-read">TO READ</option><option value="read">READ</option>':'<option value="to-watch">TO WATCH</option><option value="watched">WATCHED</option>';$("#libraryPreview").classList.add("hidden");$("#libraryPreviewImg").src="";$("#libraryFetchMessage").textContent="";f.dataset.data=""}
$("#libraryFetchBtn").onclick=async()=>{const url=$("#libraryUrl").value.trim();if(!url){$("#libraryFetchMessage").textContent="링크를 먼저 입력해 주세요.";return}const btn=$("#libraryFetchBtn");btn.disabled=true;btn.textContent="불러오는 중…";try{const d=await fetchLibraryMeta(url,libraryTypeFromDialog());$("#libraryForm").dataset.data=JSON.stringify(d);if(d.title)$("#libraryKoreanTitle").value=d.title;if(d.creator)$("#libraryManualCreator").value=d.creator;if(d.image){$("#libraryManualImage").value=d.image;$("#libraryPreviewImg").src=d.image;$("#libraryPreview").classList.remove("hidden")}$("#libraryFetchMessage").textContent="정보를 가져왔어요. 필요한 부분은 직접 수정할 수 있어요."}catch(e){console.error(e);$("#libraryFetchMessage").textContent="자동 인식 실패 · 아래에 직접 입력해 주세요."}finally{btn.disabled=false;btn.textContent="링크에서 정보 가져오기"}};
function saveLibraryRecord(type){const url=safeUrl($("#libraryUrl").value.trim()),ko=cleanText($("#libraryKoreanTitle").value),foreign=cleanText($("#libraryForeignTitle").value),creator=cleanText($("#libraryManualCreator").value),rawImage=cleanText($("#libraryManualImage").value);if(!ko&&!foreign){alert("한국어 제목 또는 외국어 제목을 하나 이상 입력해 주세요.");return null}return{id:Date.now(),url,title:ko||foreign,altTitle:ko&&foreign?foreign:"",titleKo:ko,titleForeign:foreign,creator,image:safeUrl(rawImage)||"assets/flower.jpg",status:$("#libraryStatus").value,createdAt:Date.now()}}
function openLibraryDialog(type){resetLibraryForm(type);const d=$("#libraryDialog");try{d.showModal()}catch{d.setAttribute("open","")}setTimeout(()=>$("#libraryUrl")?.focus(),60)};
$("#openBook").onclick=()=>openLibraryDialog("books");$("#openMovie").onclick=()=>openLibraryDialog("movies");$("#libraryForm").onsubmit=async e=>{e.preventDefault();const type=libraryTypeFromDialog(),rec=saveLibraryRecord(type);if(!rec)return;S[type].unshift(rec);localBackup();renderLibrary(type);$("#libraryDialog").close();try{await cloudSave()}catch(err){console.error(err);updateSync("기기에 저장됨")}};
