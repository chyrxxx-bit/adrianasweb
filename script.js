...
async function googleBooksSearch(q){
 ...
 if(!r.ok)throw Error("book search failed");const j=await r.json();
...
async function searchScreenSource(q){
 const r=await fetch(LIBRARY_ENDPOINTS.movies(q),{cache:"no-store"});if(!r.ok)throw Error("screen search failed");return r.json();
}
