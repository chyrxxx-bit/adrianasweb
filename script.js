...
// ---------- Books / Movies search ----------
// IMPORTANT: the two search modes are intentionally isolated.
// BOOKS -> Google Books only, with a title-only relevance gate.
// MOVIES -> IMDb title suggestions + Apple catalog, restricted to movies and TV-series/drama titles.
// We never accept people, songs, podcasts, apps, books, etc.
// Korean queries are sent unchanged to the KR catalog first, so Korean titles can be found too.
const LIBRARY_ENDPOINTS={
  books:(q)=>`https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(q)}&maxResults=40&printType=books&orderBy=relevance`,
  movies:(q)=>`https://v3.sg.media-imdb.com/suggestion/titles/x/${encodeURIComponent(q)}.json`,
  moviesApple:(q,country="KR")=>`https://itunes.apple.com/search?term=${encodeURIComponent(q)}&entity=movie&country=${country}&limit=50&media=movie`
};
...
function resetLibraryForm(type){
 ...
 $("#libraryModalLabel").textContent=type==="books"?"BOOK":"MOVIE / DRAMA";$("#libraryModalTitle").textContent=type==="books"?"책 추가":"영화 / 드라마 추가";
 ...
 $("#libraryResults").innerHTML="";$("#libraryResults").classList.remove("has-results");$("#libraryMeta").classList.add("hidden");$("#libraryCoverChoices").classList.add("hidden");$("#libraryCoverChoices").innerHTML="";$("#libraryFetchMessage").textContent="";f.dataset.selected="";
}
function cleanText(v){return String(v||"").replace(/\s+/g," ").trim()}
function coverUrl(v){if(!v)return "";return String(v).replace(/^http:/,"https:")}
...
function titleTokens(v){return cleanText(v).toLowerCase().split(/\s+/).map(x=>x.replace(/[^0-9a-z\uac00-\ud7a3]/g,"")).filter(Boolean)}
...
function titleScore(title,q){
 ...
 if(t===n)return 1200;
 if(t.includes(n))return 1000-Math.min(300,Math.max(0,t.length-n.length));
 if(n.includes(t) && t.length>=2)return 920-Math.min(250,n.length-t.length);
 const qt=titleTokens(q),tt=titleTokens(title);
 let hits=0;
 for(const x of qt)if(x.length>=2&&tt.some(y=>y.includes(x)||x.includes(y)))hits++;
 if(hits===qt.length&&hits>0)return 780+hits*30;
 if(hits>0)return 500+hits*40;
 // Small typo tolerance. This is deliberately applied to titles only.
 if(Math.min(t.length,n.length)>=3){
  const d=editDistance(t,n),ratio=1-d/Math.max(t.length,n.length);
  if(ratio>=0.62)return 420+Math.round(ratio*100);
 }
 // Character n-gram overlap helps searches such as "해리포" -> "해리 포터".
 if(Math.min(t.length,n.length)>=3){
  const grams=v=>new Set([...v].map((_,i)=>v.slice(i,i+2)).filter(x=>x.length===2));
  const a=grams(t),b=grams(n);let inter=0;for(const x of a)if(b.has(x))inter++;
  const union=new Set([...a,...b]).size;const j=union?inter/union:0;
  if(j>=0.45)return 360+Math.round(j*100);
 }
 return 0;
}
function uniqueById(arr){const m=new Map();for(const x of arr){if(!x?.id)continue;if(!m.has(x.id)||x._score>(m.get(x.id)._score||0))m.set(x.id,x)}return [...m.values()]}
function dedupeLibraryResults(arr){
 const m=new Map();
 for(const x of arr){if(!x?.title)continue;const k=normalizeTitle(x.title);if(!k)continue;const old=m.get(k);if(!old||((x._score||0)>(old._score||0))||(!old.image&&x.image))m.set(k,x)}
 return [...m.values()].sort((a,b)=>(b._score||0)-(a._score||0)||a.title.localeCompare(b.title));
}
async function googleBooksSearch(q){
 const r=await fetch(LIBRARY_ENDPOINTS.books(q),{cache:"no-store"});if(!r.ok)throw Error("book search failed");const j=await r.json();
 return (j.items||[]).filter(x=>x?.kind==="books#volume"&&x?.volumeInfo?.printType!=="MAGAZINE").map(x=>{
   const i=x.volumeInfo||{},im=i.imageLinks||{};const title=cleanText(i.title);
   return {id:`gbook-${x.id}`,type:"books",title,subtitle:cleanText(i.subtitle),creator:cleanText((i.authors||[]).join(", ")),image:coverUrl(im.extraLarge||im.large||im.medium||im.thumbnail||im.smallThumbnail),altTitle:"",source:"Google Books",published:cleanText(i.publishedDate),coverChoices:[coverUrl(im.extraLarge||im.large||im.medium||im.thumbnail||im.smallThumbnail)].filter(Boolean)};
 }).filter(x=>x.title);
}
async function searchBooks(q){
 const queries=[q];
 const compact=cleanText(q).replace(/\s+/g,"");if(compact&&compact!==q)queries.push(compact);
 const parts=titleTokens(q);if(parts.length>1)queries.push(parts.join(" "));
 for(const part of parts)if(part.length>=2)queries.push(part);
 let raw=[];
 for(const term of [...new Set(queries)]){try{raw.push(...await googleBooksSearch(term))}catch(e){console.warn("Google Books",e)}}
 raw=uniqueById(raw).map(x=>({...x,_score:titleScore(x.title,q)})).filter(x=>x._score>=360);
 raw=dedupeLibraryResults(raw).slice(0,12);
 const groups=new Map();
 for(const item of raw){const k=normalizeTitle(item.title);if(!groups.has(k))groups.set(k,[]);groups.get(k).push(item)}
 return [...groups.values()].map(group=>{const first=group[0];first.coverChoices=[...new Set(group.map(x=>x.image).filter(Boolean))].slice(0,5);return first});
}
async function searchImdbMovies(q){
 const r=await fetch(LIBRARY_ENDPOINTS.movies(q),{cache:"no-store"});if(!r.ok)throw Error("IMDb search failed");const j=await r.json();
 // IMDb suggestion results include movies, TV series, and mini-series.
 // Keep only actual screen works; exclude people, characters, episodes, games, etc.
 const allowed=new Set(["movie","tvSeries","tvMiniSeries","tvMovie"]);
 return (j.d||[]).filter(x=>x?.id?.startsWith("tt")&&allowed.has(x.qid||x.q)).map(x=>({
   id:`imdb-${x.id}`,type:"movies",title:cleanText(x.l),altTitle:"",creator:cleanText(x.s||""),image:coverUrl(x.i?.imageUrl||(Array.isArray(x.i)?x.i[0]:"")),coverChoices:[coverUrl(x.i?.imageUrl||(Array.isArray(x.i)?x.i[0]:""))].filter(Boolean),source:"IMDb",year:x.y?String(x.y):"",imdbId:x.id,workType:(x.qid||x.q)==="movie"?"movie":"tv"
 })).filter(x=>x.title);
}
async function searchItunesMovies(q,country){
 const r=await fetch(LIBRARY_ENDPOINTS.moviesApple(q,country),{cache:"no-store"});if(!r.ok)throw Error("Apple movie search failed");const j=await r.json();
 return (j.results||[]).filter(x=>x?.wrapperType==="track"&&x?.kind==="feature-movie"&&x?.trackName).map(x=>({
   id:`itunes-${country}-${x.trackId}`,type:"movies",title:cleanText(x.trackName),altTitle:"",creator:cleanText(x.artistName),image:coverUrl(x.artworkUrl100||x.artworkUrl60),coverChoices:[coverUrl(x.artworkUrl100||x.artworkUrl60)].filter(Boolean),source:"Apple Movies",year:x.releaseDate?String(x.releaseDate).slice(0,4):""
 })).filter(x=>x.title);
}
async function searchMovies(q){
 // Keep the original Korean/English query intact, plus a compact form for spacing differences.
 // IMDb covers movies + TV series/mini-series; Apple KR/US is a second catalog fallback.
 const queries=[q];const compact=cleanText(q).replace(/\s+/g,"");if(compact&&compact!==q)queries.push(compact);
 const parts=titleTokens(q);if(parts.length>1)queries.push(parts.join(" "));
 let raw=[];
 for(const term of [...new Set(queries)]){
   try{raw.push(...await searchImdbMovies(term))}catch(e){console.warn("IMDb",e)}
   for(const country of ["KR","US"]){try{raw.push(...await searchItunesMovies(term,country))}catch(e){console.warn("Apple Movies",e)} }
 }
 // Relevance is checked against the work title only, so unrelated cast/creator matches are rejected.
 raw=uniqueById(raw).map(x=>({...x,_score:titleScore(x.title,q)})).filter(x=>x._score>=360);
 raw=dedupeLibraryResults(raw).slice(0,16);
 const groups=new Map();
 for(const item of raw){const k=normalizeTitle(item.title);if(!groups.has(k))groups.set(k,[]);groups.get(k).push(item)}
 return [...groups.values()].map(group=>{const first=group[0];first.coverChoices=[...new Set(group.map(x=>x.image).filter(Boolean))].slice(0,5);return first});
}
function renderLibraryResults(results,type){
 const box=$("#libraryResults");box.classList.add("has-results");box.innerHTML=results.length?results.map((r,i)=>`<button type="button" class="library-result" data-result="${i}"><img src="${escapeAttr(r.image||"assets/flower.jpg")}" onerror="this.src='assets/flower.jpg'"><span><strong>${escapeHtml(r.title)}</strong>${r.altTitle?`<small>${escapeHtml(r.altTitle)}</small>`:""}<em>${escapeHtml(r.creator||"정보 확인 필요")}</em></span></button>`).join(""):'<div class="empty">검색 결과가 없습니다.</div>';
 box.querySelectorAll(".library-result").forEach((el,i)=>el.onclick=()=>selectLibraryResult(results[i],type));box._results=results;
}
function renderCoverChoices(item){
 const box=$("#libraryCoverChoices"),imgs=[...new Set((item.coverChoices||[]).filter(Boolean))];
 if(!imgs.length){box.classList.add("hidden");box.innerHTML="";return}
 box.classList.remove("hidden");box.innerHTML='<span>표지 / 포스터 선택</span>'+imgs.map((im,i)=>`<button type="button" class="cover-choice ${i===0?"active":""}" data-image="${escapeAttr(im)}"><img src="${escapeAttr(im)}" onerror="this.parentElement.style.display='none'"></button>`).join("");
 box.querySelectorAll(".cover-choice").forEach(b=>b.onclick=()=>{box.querySelectorAll(".cover-choice").forEach(x=>x.classList.remove("active"));b.classList.add("active");$("#libraryManualImage").value=b.dataset.image;$("#libraryPreviewImg").src=b.dataset.image});
 $("#libraryManualImage").value=imgs[0]||"";
}
function selectLibraryResult(item,type){
 $("#libraryForm").dataset.selected=JSON.stringify(item);$("#libraryManualCreator").value=item.creator||"";$("#libraryPreviewImg").src=item.image||"assets/flower.jpg";$("#libraryPreviewTitle").textContent=item.title||"";$("#libraryPreviewAlt").textContent=item.altTitle||"";$("#libraryPreviewCreator").textContent=item.creator||"";$("#libraryMeta").classList.remove("hidden");renderCoverChoices(item);$("#libraryFetchMessage").textContent="작품을 선택했어요.";
}
async function runLibrarySearch(){
 const type=libraryTypeFromDialog(),q=$("#libraryQuery").value.trim();if(!q){$("#libraryFetchMessage").textContent="제목을 입력해 주세요.";return}
 const btn=$("#librarySearchBtn");btn.disabled=true;btn.textContent="검색 중…";$("#libraryFetchMessage").textContent="";$("#libraryResults").innerHTML="";
 try{const results=type==="books"?await searchBooks(q):await searchMovies(q);renderLibraryResults(results,type);$("#libraryFetchMessage").textContent=results.length?`${results.length}개의 작품을 찾았어요. 원하는 작품을 선택해 주세요.`:"해당 제목과 관련된 작품을 찾지 못했어요."}catch(e){console.error(e);$("#libraryFetchMessage").textContent="자동 검색에 실패했어요. 다시 검색해 주세요."}finally{btn.disabled=false;btn.textContent="자동 검색"}
}
function openLibraryDialog(type){resetLibraryForm(type);const d=$("#libraryDialog");try{d.showModal()}catch{d.setAttribute("open","")}setTimeout(()=>$("#libraryQuery")?.focus(),60)}
$("#openBook").onclick=()=>openLibraryDialog("books");$("#openMovie").onclick=()=>openLibraryDialog("movies");$("#librarySearchBtn").onclick=runLibrarySearch;$("#libraryQuery").addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();runLibrarySearch()}});
$("#libraryForm").onsubmit=async e=>{e.preventDefault();const type=libraryTypeFromDialog(),raw=$("#libraryForm").dataset.selected;let item={};try{item=JSON.parse(raw||"{}")}catch{}const title=cleanText(item.title)||cleanText($("#libraryQuery").value),creator=cleanText($("#libraryManualCreator").value)||item.creator||"",image=cleanText($("#libraryManualImage").value)||item.image||"assets/flower.jpg";if(!title){alert("제목을 입력해 주세요.");return}const rec={id:Date.now(),title,altTitle:item.altTitle||"",creator,image,status:$("#libraryStatus").value,createdAt:Date.now()};S[type].unshift(rec);await cloudSave();$("#libraryDialog").close();renderLibrary(type)};
function librarySearchMatch(item,q){const z=(item.title+" "+(item.altTitle||"")+" "+(item.creator||"")).toLowerCase();return z.includes(q)}
function renderLibrary(type){const status=S.libraryStatus[type],q=$(type==="books"?"#bookSearch":"#movieSearch").value.trim().toLowerCase(),arr=S[type].filter(x=>x.status===status&&(!q||librarySearchMatch(x,q))),grid=$(type==="books"?"#bookGrid":"#movieGrid");grid.innerHTML=arr.length?arr.map(x=>`<article class="wish library-card"><img class="wish-img" src="${escapeAttr(x.image||"assets/flower.jpg")}" onerror="this.src='assets/flower.jpg'"><div class="wish-body"><h3>${escapeHtml(x.title)}</h3>${x.altTitle?`<div class="library-alt-title">${escapeHtml(x.altTitle)}</div>`:""}<div class="library-creator">${escapeHtml(x.creator||"정보 없음")}</div><div class="wish-meta"><span class="badge">${escapeHtml(status.toUpperCase().replace("-"," "))}</span><button class="delete-btn" data-library-delete="${x.id}" data-library-type="${type}">삭제</button></div></div></article>`).join(""): '<p class="empty">등록된 작품이 없습니다.</p>';grid.querySelectorAll("[data-library-delete]").forEach(b=>b.onclick=async()=>{if(!confirm("이 작품을 삭제할까요?"))return;S[type]=S[type].filter(x=>x.id!=b.dataset.libraryDelete);await cloudSave();renderLibrary(type)})}
