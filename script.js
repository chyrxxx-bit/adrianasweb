...
function colorForEvent(e){
 const palette=["#cf747b","#6f9b7d","#c7964f","#8d7aaa","#b87963","#5f9791"];
...
function renderCal(){
 ...
 box.innerHTML=es.length?es.map((e,i)=>`<div class="event" data-i="${i}"><strong>${escapeHtml(e.title)}</strong><small>${e.startTime&&e.endTime?`${e.startTime}–${e.endTime}`:(e.startTime||e.endTime||"시간 없음")}${e.startDate&&e.endDate&&e.startDate!==e.endDate?` · ${formatShort(e.startDate)}–${formatShort(e.endDate)}`:""}</small></div>`).join(""): '<div class="empty">일정이 없습니다.</div>';
...
function openEventDialog(){
 const selected=dateKey(S.selected||new Date());$("#eventDate").value=selected;$("#eventEndDate").value=selected;$("#eventStartTime").value="";$("#eventEndTime").value="";$("#eventTitle").value="";$("#eventDialog").showModal();setTimeout(()=>$("#eventTitle").focus(),60);
}
