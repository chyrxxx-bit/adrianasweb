...
const EVENT_PALETTE=["#BFD8C2","#E9B8C5","#B9CEE8","#E8D2A6","#C9B9DF","#F0C0A8","#AFCFD0","#D8C6B8"];
function eventColorMap(){
 const events=allCalendarEvents();
 const map=new Map();
 // 겹치는 일정끼리는 같은 색을 쓰지 않도록, 각 일정과 겹치는 일정의 색을 피해 배정합니다.
 const overlaps=(a,b)=>{
  const as=new Date(eventStart(a)+"T00:00:00"), ae=new Date(eventEnd(a)+"T00:00:00");
  const bs=new Date(eventStart(b)+"T00:00:00"), be=new Date(eventEnd(b)+"T00:00:00");
  return as<=be && bs<=ae;
 };
 events.sort((a,b)=>eventStart(a).localeCompare(eventStart(b)) || eventEnd(a).localeCompare(eventEnd(b)) || String(a.title).localeCompare(String(b.title)));
 for(const e of events){
  const used=new Set();
  for(const other of events){
   if(other===e) continue;
   const c=map.get(eventIdentity(other));
   if(c && overlaps(e,other)) used.add(c);
  }
  const color=EVENT_PALETTE.find(c=>!used.has(c)) || EVENT_PALETTE[map.size % EVENT_PALETTE.length];
  map.set(eventIdentity(e),color);
 }
 return map;
}
