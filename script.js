...
// MOVIES is one combined library for films, dramas, and TV series.
// ---------- Books / Movies search ----------
...
const LIBRARY_ENDPOINTS={
  ...
  movies:(q)=>`/.netlify/functions/library-search?type=movies&q=${encodeURIComponent(q)}`,
  moviesApple:(q,country="KR")=>`/.netlify/functions/library-search?type=movies&q=${encodeURIComponent(q)}`
};
...
async function googleBooksSearch(q){
 const r=await fetch(`/.netlify/functions/library-search?type=books&q=${encodeURIComponent(q)}`,{cache:"no-store"});
 if(!r.ok){let d="";try{d=(await r.json()).detail||""}catch{}throw Error("book search failed "+r.status+" "+d)}const j=await r.json();
...
async function searchBooks(q){
 ...
 let raw=[];for(const term of [...new Set(queries)]){try{raw.push(...await googleBooksSearch(term))}catch(e){console.warn("Google Books",e)}}
 raw=uniqueById(raw).map(x=>({...x,_score:titleScore(x.title,q)})).filter(x=>x._score>=300);
 return dedupeLibraryResults(raw).slice(0,16);
}
async function searchScreenSource(q){
 const r=await fetch(LIBRARY_ENDPOINTS.movies(q),{cache:"no-store"});if(!r.ok){let d="";try{d=(await r.json()).detail||""}catch{}throw Error("screen search failed "+r.status+" "+d)}return r.json();
}
function normalizeScreenItems(j){
 const out=[];
 const allowed=new Set(["movie","tvSeries","tvMiniSeries","tvMovie","feature","TV series","TV mini-series","TV movie"]);
 for(const x of (j.imdb||[])){
   const kind=x.qid||x.q; if(!x?.id?.startsWith("tt")||!allowed.has(kind)||!x.l)continue;
   const image=coverUrl(x.i?.imageUrl||"");
   out.push({id:`imdb-${x.id}`,type:"movies",title:cleanText(x.l),creator:cleanText(x.s||""),image,coverChoices:image?[image]:[],source:"IMDb",year:x.y?String(x.y):"",imdbId:x.id,workType:(kind==="movie"||kind==="feature")?"movie":"tv"});
 }
 for(const [country,items] of [["KR",j.appleKR||[]],["US",j.appleUS||[]]]) for(const x of items){
   if(x?.wrapperType!=="track"||x?.kind!=="feature-movie"||!x?.trackName)continue;
   const image=coverUrl(x.artworkUrl100||x.artworkUrl60||"");
   out.push({id:`itunes-${country}-${x.trackId}`,type:"movies",title:cleanText(x.trackName),creator:cleanText(x.artistName||""),image,coverChoices:image?[image]:[],source:"Apple Movies",year:x.releaseDate?String(x.releaseDate).slice(0,4):"",workType:"movie"});
 }
 for(const x of (j.tvmaze||[])){
   const show=x?.show;if(!show?.id||!show?.name)continue;
   const image=coverUrl(show.image?.original||show.image?.medium||"");
   const creator=cleanText((show.network?.name||show.webChannel?.name||""));
   out.push({id:`tvmaze-${show.id}`,type:"movies",title:cleanText(show.name),creator, image, coverChoices:image?[image]:[], source:"TV", year:show.premiered?String(show.premiered).slice(0,4):"", workType:"tv"});
 }
 return out;
}
...
async function searchMovies(q){
 ...
 const parts=titleTokens(q);if(parts.length>1)queries.push(parts.join(" "));
 let raw=[];
 for(const term of [...new Set(queries)]){try{raw.push(...normalizeScreenItems(await searchScreenSource(term)))}catch(e){console.warn("Screen search",e)}}
 raw=uniqueById(raw).map(x=>({...x,_score:titleScore(x.title,q)})).filter(x=>x._score>=300);
 return dedupeLibraryResults(raw).slice(0,20);
}
